...
class SkySys {
  ...
  /* 25.. seems to be good limit for clear sky. ~60 seconds.
     .. maybe better 26? */
  this.snrLim = 25;
  this.snr_history = [ 10, 11, 12, 13, 14, 13, 14, 15, 18, 19, 
  ...
  getSatSNR(n) { /* Get n-th strongest sat */
...
let sky = {
  ...
  init: function () {
    ...
    this.all.init();
    if (this.split) {
      this.s_gp = deepCopy(this.all);
      this.s_gl = deepCopy(this.all);
      this.s_bd = deepCopy(this.all);
    }
  },
